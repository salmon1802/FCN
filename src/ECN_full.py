...
class ECN_full(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(ECN_full, self).__init__(feature_map,
                                  model_id=model_id,
                                  gpu=gpu,
...
class PyramidalExponentialCrossNetwork(nn.Module):
    def __init__(self,
                 input_dim,
                 num_cross_layers=3,
                 batch_norm=True,
                 net_dropout=0.1,
                 num_heads=1):
        super(PyramidalExponentialCrossNetwork, self).__init__()
        self.num_cross_layers = num_cross_layers
        self.layer_norm = nn.ModuleList()
        self.batch_norm = nn.ModuleList()
        self.dropout = nn.ModuleList()
        ...
        for i in range(num_cross_layers):
            if input_dim % (2 ** i) == 0:
                self.max_i = i
            self.w.append(nn.Linear(input_dim, input_dim // (2 ** self.max_i), bias=False))
            self.b.append(nn.Parameter(torch.empty((input_dim,))))
            if batch_norm:
                self.batch_norm.append(nn.BatchNorm1d(num_heads))
            if net_dropout > 0:
    ...
    def forward(self, x):
        for i in range(self.num_cross_layers):
            if i > self.max_i:
                H = self.w[i](x).repeat(1, 1, 2 ** self.max_i)
            else:
                H = self.w[i](x).repeat(1, 1, 2 ** i)
            if len(self.batch_norm) > i:
                H = self.batch_norm[i](H)
            x = x * (H + self.b[i]) + x
            if len(self.dropout) > i:
